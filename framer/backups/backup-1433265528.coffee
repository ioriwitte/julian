...
hearts = ["blue", "orange", "purple", "red", "teal", "yellow"]

# Video courtesy of the Earth Science and Remote Sensing Unit, NASA Johnson Space Center
# http://eol.jsc.nasa.gov/
videoLayer = new VideoLayer 
	x:0, y:230, width:1624, height:1080, video:"images/ffc-space.mp4"
...
playPauseBtn = new Layer
	x: 642, y: 1200, borderRadius: "50%", backgroundColor: "transparent"	
inputLabel = new Layer
	x: 20, y: 1212, height: 80, width: 610, borderRadius: "10px", backgroundColor: "rgba(255, 255, 255, 0.24)"
...
firstLine = new Layer
	width: lineWidth, height: lineHeight, x:35, backgroundColor:'#ffffff', borderRadius:'3px'
secondLine = new Layer
	width: lineWidth, height: lineHeight, x:55, backgroundColor:'#ffffff', borderRadius:'3px'
thirdLine = new Layer
	width: lineWidth, height: lineHeight, x:75, backgroundColor:'#ffffff', borderRadius:'3px', opacity: 0, rotation: 60
...
###
	...
	heart = new Layer 
		x: event.offsetX, y: event.offsetY, width: 81, height: 72, image: "images/#{random}.png"
	heartStart = new Animation
		layer: heart
		properties:
			x: event.offsetX + Utils.randomNumber(-50, 50)
			y: event.offsetY - moveY
			opacity: 0.5
